...
Finally, we have two additional optional arguments. The first of these arguments can be used to control the number of groups to be formed for the GATES analysis (details below), with the default number equal to five. The second of these arguments can be used to prevent the `evalue_cates` function from printing the status of progress on the console. We use the default of five groups and prevent the function from printing the progresses.  
...
with the model fitted via OLS, and $\hat{\Gamma}_i := \hat{\mu}_1 ( X_i ) - \hat{\mu}_0 ( X_i ) + \frac{D_i [ Y_i - \hat{\mu}_1 ( X_i ) ]}{\hat{p} ( X_i )} - \frac{[ 1 - D_i ] [ Y_i - \hat{\mu}_0 ( X_i ) ]}{1 - \hat{p} ( X_i )}$. The doubly-robust scores $\Gamma_i$ are estimated internally using the the validation sample and 5-fold cross-fitting. Results can be accessed by calling `evaluation$BLP$aipw`.
...
### Compare Results
Our package is compatible with the generic S3 methods `print`, `summary`, and `plot` that allow the user to investigate the results.[^7] This section shows an example of how these methods can be used to analyze the output of a single call of the `evalue_cates` function. 
...
- **GATES**
The `plot` method allows the user to visualize the results of GATES estimation. The method plots the estimated average treatment effect of each group ($\{ \hat{\gamma}_k \}_{k = 1}^K$) for all the methodologies detailed above. Error bars displaying 95% confidence intervals are also displayed.

```{r true-gates, eval = TRUE}
## GATES results.
plot(evaluation)
```
...
[^7]: The `print` and `summary` methods return the same output in our package.
