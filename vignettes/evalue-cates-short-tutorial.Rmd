...
After handling the inputs and running some checks, the `evalue_cates` function estimates the BLP and the GATES using only the validation sample. In the following, we detail the main results returned by the function. 
...
The second object estimated by the `evalue_cates` function are the sorted group average treatment effects, with the groups formed by cutting the distribution of $\hat{\tau} ( \cdot )$ into $K$ bins:[^7]
...
The `evalue_cates` function estimates the GATES using the same three strategies used for the BLP estimation. As before, all linear models are fitted using only observations in the validation sample. Additionally, `evalue_cates` also implements a nonparametric estimator of the GATES.
...
SKETCH IMAI AND LI HERE.

[^1]: Complete references to these papers are listed in the home page.
[^2]: If we do not use the `pscore` argument at all, the `evalue_cates` function will estimate the propensity scores internally via a honest regression forest constructed using only the training sample. However, notice that most methodologies implemented here are valid only under randomized experiments, where $p ( \cdot )$ is known.
