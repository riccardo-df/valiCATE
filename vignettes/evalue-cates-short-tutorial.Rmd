...
- $w ( X_i ) = \frac{1}{\hat{p} ( X_i ) [ 1 - \hat{p} ( X_i )]}$ &rarr; Propensity score weights;
...
After handling the inputs and running some checks, the `evalue_cates` function estimates the BLP and the GATES using only the validation sample. In the following, we detail this process.

#### Best Linear Predictor
The first object targeted by the `evalue_cates` function is the BLP of the actual CATEs using the estimated CATEs:
...
with the model fitted via WLS using weights $w ( X_i)$, and $\mathbb{E}_{n, V}$ denoting the sample average with respect to the validation sample. Results can be accessed by calling `evaluation$BLP$wr_none`.

- **Horvitz-Thompson**

$$ H_i Y_i = \beta_1 + \beta_2 \{ \hat{\tau} ( X_i ) - \mathbb{E}_{n, V} [ \hat{\tau} ( X_i ) ] \} + \epsilon_i $$
with the model fitted via OLS. Results can be accessed by calling `evaluation$BLP$ht_none`.

- **AIPW**

$$ \hat{\Gamma}_i = \beta_1 + \beta_2 \{ \hat{\tau} ( X_i ) - \mathbb{E}_{n, V} [ \hat{\tau} ( X_i ) ] \} + \epsilon_i $$
with the model fitted via OLS, and $\hat{\Gamma}_i := \hat{\mu}_1 ( X_i ) - \hat{\mu}_0 ( X_i ) + \frac{D_i [ Y_i - \mu_1 ( X_i ) ]}{\hat{p} ( X_i )} - \frac{[ 1 - D_i ] [ Y_i - \hat{\mu}_0 ( X_i ) ]}{1 - \hat{p} ( X_i )}$.[^6] Results can be accessed by calling `evaluation$BLP$aipw`.

### Sorted Group Average Treatment Effects
The second object targeted by the `evalue_cates` function are the group average treatment effects, with the groups formed by cutting the distribution of $\hat{\tau} ( \cdot )$ into $K$ bins:[^7]

$$ \gamma_k := \mathbb{E} [ \tau ( X_i ) | \ell_{k - 1} \leq \hat{\tau} ( X_i ) < l_k ], \,\,\, k = 1, \dots, K$$
with $- \infty = \ell_0 < \ell_1 < \dots < \ell_K < + \infty$. The GATES are of interest for two main reasons:

- First, they provided a nonlinear predictor of $\tau ( \cdot )$, as they provide the BLP of $\tau ( \cdot )$ using the group indicators $\mathbb{1} ( \ell_{k - 1} \leq \hat{\tau} ( X_i ) < l_k )$ (Chernozhukov et al., 2017);
...
The `evalue_cates` function estimates the GATES using the same three strategies used for the BLP estimation. As before, all linear models are fitted using only observations in the validation sample.

- **Weighted Residuals**

$$ Y_i = \sum_{k = 1}^K \gamma_k [ D_i - p ( X_i ) ] \mathbb{1} ( \ell_{k - 1} \leq \hat{\tau} ( X_i ) < l_k ) + \epsilon_i $$
with the model fitted via WLS using weights $w ( X_i)$. Results can be accessed by calling `evaluation$GATES$wr_none`.

- **Horvitz-Thompson**

$$ H_i Y_i = \sum_{k = 1}^K \gamma_k \mathbb{1} ( \ell_{k - 1} \leq \hat{\tau} ( X_i ) < l_k ) + \epsilon_i $$
with the model fitted via OLS. Results can be accessed by calling `evaluation$GATES$ht_none`.

- **AIPW**

$$ \Gamma_i = \sum_{k = 1}^K \gamma_k \mathbb{1} ( \ell_{k - 1} \leq \hat{\tau} ( X_i ) < l_k ) + \epsilon_i $$
with the model fitted via OLS. Results can be accessed by calling `evaluation$GATES$aipw`.
...
[^5]: Additional constructed covariates which are not necessary for identifying the targets but can significantly reduce the variance of the estimation can be included in the regressions. For each of the strategies detailed above, the `evalue_cates` function fits several linear model, each including a different set of additional covariates. Details can be found in the [denoising vignette](https://riccardo-df.github.io/evalueCATEs/articles/denoising.html).
[^6]: The doubly-robust scores $\Gamma_i$ are estimated internally using the the validation sample and 5-fold cross-fitting.
[^7]: The $K$ bins can be defined in various ways. Here we follow the approach of (Chernozhukov et al., 2017) and form the groups by cutting the distribution of $\hat{\tau} ( X_i )$ into $K$ quantiles.
