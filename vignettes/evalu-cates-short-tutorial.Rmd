...
## Motivating Example
We illustrate the purpose of the `evaluCATE` package through the following example, where we generate simulated data. In this data, we have only one covariate $X_{i, 1} \sim \mathcal{U} ( 0, 1 )$, the treatment is randomly allocated as in a Bernoulli experiment, and treatment effects are zero for all units. 
...
data.frame("x1" = X, "true_cates" = mu1 - mu0, "estimated_cates" = cates) %>%
  ...
  xlab("X1") + ylab("CATEs") +  
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5), legend.position = c(0.12, 0.85), legend.text = element_text(size = 8), legend.key.size = unit(0.4, 'cm'))
```
...
with the model fitted via OLS and the doubly-robust scores $\hat{\Gamma}_i$ estimated using the validation sample and 5-fold cross-fitting.
...
All the GATES confidence intervals include the zero, suggesting that no group is affected by the treatment (true in our example!). The $p$-values attached to testing the hypotheses that al thrl GATES are equal and that the most affected and the least affected groups feature the same response to the treatment are large and lead to a failure in rejecting these hypotheses. To better digest GATES point estimates and confidence intervals, we can use the `plot` method setting the `target` argument to `"GATES"`.
