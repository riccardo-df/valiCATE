...
1. a training sample for estimation of the CATEs, and
...
The BLP, GATES, and RATEs can be estimated by calling the `evaluCATE` function. When calling this function, we need to supply our sample using the first six arguments: `Y_tr`, `Y_val`, `D_tr`, `D_val`, `X_tr`, and `X_val`, corresponding to $Y_i$, $D_i$, and $X_i$ separately for the training and validation subsamples. Additionally, we must supply our CATE predictions on the validation sample $\hat{\tau} ( X_i )$ by using the `cates_val` argument. Be careful, as these predictions must be produced by a model estimated using only the training sample, as we did in the Motivating Example section.

The `evaluCATE` function is able to implement all the BLP and GATES identification and estimation strategies outlined in the Quality Evaluation section. The user can choose their preferred strategies by controlling the `strategies` argument. This has no impact on RATEs estimation, which are always estimated using the sample-averaging estimators discussed above. Moreover, the nonparametric strategy for GATES estimation is always implemented. Here we decide to implement all the three possible strategies.

As discussed in the [denoising vignette](https://riccardo-df.github.io/evaluCATE/articles/denoising.html), we can add various sets of constructed covariates in our regressions to reduce the variance of the estimation. The user can choose whether to add these additional covariates by controlling the `denoising` argument. This has no impact on RATEs estimation and on the results from the nonparametric GATES estimation strategy. Here we decide not to add constructed covariates.

The constructed covariates depend on particular nuisance functions, e.g., propensity score and conditional mean of the outcome. The user can supply predictions on the validation sample of $p ( \cdot )$, $\mu ( \cdot )$, $\mu_0 ( \cdot )$, and $\mu_1 ( \cdot )$ by using the optional arguments `pscore_val`, `mu_val`, `mu0_val`, and `mu1_val`, respectively. Be careful, as these predictions must be produced by models estimated using only the training sample. If not provided by the user, these functions are estimated internally via honest regression forests using only the training sample. In our Motivating Example, we have knowledge of the actual propensity score, which equals $0.5$ for all units.[^9] We supply these values in the call below and let the function estimate the other nuisances internally (in this case, the function will skip the nuisance estimation as this is not required by our choice of the `denoising` argument).
...
### Results
##### **BLP**
Let us have a look at the results. The `summary` method allows us to visualize the results of the BLP estimation if we set the `target` argument to `BLP`. The `latex` argument controls whether the raw results or LATEX code for a table will be displayed in the console. We could also use the `which_models` argument to choose which results to display (useful to avoid a lengthy output). Because we have estimated a small number of models, we leave the deafult option and show everything.

```{r blp summary, eval = TRUE}
## BLP summary.
summary(evaluation, target = "BLP") # Try 'latex = TRUE'.
```

The estimated ATEs range between $0.05$ and $0.18$, with all confidence intervals covering the true value of zero. Additionally, we always fail to reject the hypothesis $\beta_2 = 0$, implying that either the effects are homogeneous or our CATE estimates are unreliable (we know both to be true in our Motivating Example).

##### **GATES**
We can also use the `summary` method with the `target` argument to `GATES` to visualize the results of the GATES estimation. The other arguments work as before.
```{r gates-summary, eval = TRUE}
## GATES summary.
# summary(evaluation, target = "GATES") # Try 'latex = TRUE'.
```

All the GATES confidence intervals include the zero, suggesting that no group is affected by the treatment (true in our Motivating Example!). The $p$-values attached to testing the hypotheses that all the GATES are equal and that the most affected and the least affected groups feature the same response to the treatment are large and lead to a failure in rejecting these hypotheses. 

To better digest GATES point estimates and confidence intervals, we can use the `plot` method setting the `target` argument to `"GATES"`. As for the `summary` method, we could use the `which_models` argument to choose which results to display (not needed here).
...
##### **RATEs**
Finally, the `summary` method with the `target` argument to `RATE` allows us to visualize the results of the RATE estimation.

```{r rates summary, eval = TRUE}
## RATEs summary.
# summary(evaluation, target = "RATE") # Try 'latex = TRUE'.
```

The results agree with BLP.

We can plot the estimated TOC curve by calling the `plot` method setting the `target` argument to `"TOC"`. 
...
[^4]: For each of these alternatives, additional constructed covariates which are not necessary for identifying the targets but can significantly reduce the variance of the estimation can be included in the regressions. Details can be found in the [denoising vignette](https://riccardo-df.github.io/evaluCATE/articles/denoising.html).
