...
For illustration, we utilize the same Motivating Example as in the [short tutorial](https://riccardo-df.github.io/evaluCATE/articles/evaluCATE-short-tutorial.html). The only distinction is that we employ several denoising strategies here.
...
### GATES
As explained in the [short tutorial](https://riccardo-df.github.io/evaluCATE/articles/evaluCATE-short-tutorial.html), we can call the `plot` method and set the `target` argument to GATES to display GATES point estimates and 95% confidence intervals. However, due to the relatively large number of models we estimated above, the results may be difficult to interpret.
